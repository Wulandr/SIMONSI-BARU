...
<?php if ($pengajuan == 0 && ($tor[$t]->nama_pic == Auth::user()->name || $RoleLogin == "Prodi" || $RoleLogin == "Admin")) { ?>
    ...
    @endcan
    @can('tor_delete')
    <a href="{{url('/tor/delete/'.base64_encode($tor[$t]->id))}}" class="button tor-confirm" data-bs-toggle="tooltip" title="Delete">
        <button class="badge badge-primary rounded">
    ...
    <?php if ($sudahAdaRAB == 0) { ?>
        <button class="search-toggle iq-waves-effect badge badge-info rounded" data-bs-toggle="modal" title="Tambah RAB" data-original-title="Tambah RAB" data-bs-target="#tambah_rab<?= $tor[$t]->id ?>">Tambah RAB<br />
        </button>
    <?php } ?>
    @endcan
        ...
        if ($rab[$sr]->id_tor == $tor[$t]->id) {
            foreach ($anggaran as $anggaran1) {
                if ($anggaran1->id_rab == $rab[$sr]->id && $buttonAjukan == 1) { ?>
                    <button class="badge badge-danger rounded" data-bs-toggle="modal" data-bs-target="#ajukan{{$tor[$t]->id}}" {{$buttonVerif == "Tidak" ? 'disabled' : ''}}>Ajukan TOR & RAB
                    </button>
    <?php $buttonAjukan += 1;
                }
            }
...
<?php } ?>
<?php if ($pengajuan >= 1) { ?>
    <button class="btn btn-success btn-sm mb-1" type="button" data-bs-toggle="modal" data-bs-target="#status{{$tor[$t]->id}}">Status
    </button>
<?php } ?>
...
<?php if ($tor[$t]->jenis_ajuan == "Perbaikan" && ($perbaikan < $revisi)) { ?>
    <button class="btn btn-danger {{$buttonVerif == 'Tidak' ? 'disabled' : '' }} btn-sm mb-1" data-bs-toggle="modal" data-bs-target="#ajukan{{$tor[$t]->id}}">Ajukan TOR & RAB
    </button>
<?php } ?>
@endcan
